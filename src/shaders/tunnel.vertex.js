...
export const TunnelVertexShader = `
  ...
  uniform vec3 uPathSpline[32];
  uniform float uPathSegments;
  ...
  vec3 getSplinePosition(float z, float roadStartZ, float totalLength) {
      float t = clamp((z - roadStartZ) / totalLength, 0.0, 1.0);
      float fIndex = t * (uPathSegments - 1.0);
      
      int i0 = int(floor(fIndex));
      int i1 = int(min(float(i0 + 1), uPathSegments - 1.0));
      float fractT = fract(fIndex);

      vec3 p0 = uPathSpline[i0];
      vec3 p1 = uPathSpline[i1];

      return mix(p0, p1, fractT);
  }
  ...
  void main() {
      ...
      vec3 pCenter = getSplinePosition(zPos, roadStartZ, roadLoop);
